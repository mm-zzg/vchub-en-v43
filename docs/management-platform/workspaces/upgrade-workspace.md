...
As shown in the figure below, there are two workspaces in the list. The workspace named "Test" has a version of 4.0.0, which is lower than the system version displayed in the lower right corner (4.1.0). Since it meets the upgrade criteria, a green upgrade arrow button is displayed next to the version number.
...
If the user selects "Upgrade" only, then after a successful upgrade, only the updated workspace named "Test" with the new version 4.1.0 will appear in the list, while the old version 4.0.0 of the "Test" workspace will be removed.
