...
The WAGO Protocol driver in WAGO SCADA supports the basic connection to WAGO PLCs. The system comes with an MQTT Broker, which supports the user's connection via the account and password provided by the system and the data communication via the WAGO Protocol driver.
...
2. Configuration of Cloud Connectivity
   ...
   - In the **Hostname** field, enter the IP address published by your WAGO SCADA program.
   ...
   - *There is no need to configure other options, as the current WAGO SCADA program does not implement them*.


   **Note**: Please do not check **Use Compression**, as this will cause the WAGO Protocol driver to fail in processing data. And for the fields not mentioned in the above list, checking these fields does not affect functionality, because WAGO SCADA has not yet implemented the corresponding features.
...
The WAGO SCADA program supports a secure connection with WAGO PLC devices via the TLS protocol.
...
Configure the client: Modify the connection configuration in the configuration page of the WAGO PLC device.
   ...
   *__Replace [server IP] with the server IP deployed by WAGO SCADA__
