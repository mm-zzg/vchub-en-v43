...
3. Get the information about the InfluxDB server, such as host, port, use SSL, user name, password, etc. Fill in the information into the configuration box and save it. (Note: The following data is only an example, please fill in according to the actual situation). 
   ![alt text](20.png)
    ...
    - Port: 8087
    - Use SSL: Enabled
          - Data is transmitted over an encrypted channel between the WAGO SCADA and the InfluxDB server, ensuring confidentiality and integrity. This prevents third parties from eavesdropping or tampering with the data, which is crucial when handling sensitive information.
